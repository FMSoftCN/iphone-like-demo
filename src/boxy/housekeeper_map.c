/*
** $Id: housekeeper_map.c 224 2007-07-03 09:38:24Z xwyan $
**
** housekeeper_map.c: The map module of the housekeeper game for 
** Feynman PMP solution.
**
...
static ptagLevel InitALevel ( int *i )
{
	...
	int k;
	int b;
#ifdef _240x320_SCREEM
    ptagLevel pSwapLevel;
    int iSwapRow = 0;
    int iSwapCol = 0;
    int *pSwapInt;
    int iSwap;
...
#ifdef _240x320_SCREEM
    pSwapLevel = (ptagLevel) calloc (1, sizeof (Level));
    pSwapLevel->row = iSwapRow;
    pSwapLevel->col = iSwapCol;
    pSwapLevel->data = (int *) calloc (iRow*iCol, sizeof (int));
    pSwapInt = pSwapLevel->data;
#endif
	//set value to pLevel->data	
	j = *i;
	for ( iRow = 0; iRow < pLevel->row; iRow++ )//行
	{
	...
	*i = j;
    ...
    {
        iSwap = 0;
        for (iRow = pLevel->row -1; iRow >= 0; iRow --)//行
        {
            pSwapInt[iSwapRow*iSwapCol + iSwap] = pInt[iRow*iCol+k];
            if (pInt[iRow*iCol+k] == B_MAN || pInt[iRow*iCol+k] == B_SAVEMAN)
            {
                pSwapLevel->manx = (pLevel->row - 1) - pLevel->many;
                pSwapLevel->many = pLevel->manx;
            }
            if (iSwap < pSwapLevel->col)
                iSwap ++;
        }
        if (iSwapRow < pSwapLevel->row)
            iSwapRow ++;
    }
    return pSwapLevel;
#else
...
int Housekeeper_GotoCollection ( ptagLevelCollection pColl )
{
...
void Housekeeper_DestroyMap ( void )
{
...
// ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  == =
//Play Functions:
int Housekeeper_PlayMove ( HWND hwnd, int dx, int dy )
{
	...
	Housekeeper_PushStep ( pAStep );
	//act
	cur_level->data[cury  * iCol + curx]  = curafter;
	cur_level->data[dy * iCol + dx] = nextafter;
	Housekeeper_DrawALittleBlock ( hwnd, curx, cury, curafter );	
	Housekeeper_DrawALittleBlock ( hwnd, dx, dy, nextafter );	
...
int Housekeeper_PlayKeyboard ( HWND hwnd, int iDir )
{
	int curx,cury;
	int nextx,nexty;
	int farx,fary;
	...
	if ( nextafter != B_FORBID ){//move to
		...
		Housekeeper_DrawALittleBlock ( hwnd, curx, cury, curafter );	
		Housekeeper_DrawALittleBlock ( hwnd, nextx, nexty, nextafter );	
	...
	if ( ( nextbefore == B_OBJECT ) || ( nextbefore == B_TREASURE ) )//try pull box
	{
		...
		Housekeeper_DrawALittleBlock ( hwnd, curx, cury, curafter );	
		Housekeeper_DrawALittleBlock ( hwnd, nextx, nexty, nextafter );	
		Housekeeper_DrawALittleBlock ( hwnd, farx, fary, farafter );	
...
void Housekeeper_PushStep ( ptagStep pStep )
{
	theMap->shead++;
	theMap->shead %= 50;
...
void Housekeeper_PlayRestart ( void )
{	
...
BOOL Housekeeper_CheckMissionComplete ( void )
{
	int i,j;
	int iRow, iCol;	
	int a;
...
int Housekeeper_PlayUndo ( HWND hwnd )
{
	ptagStep pAStep;
	ptagLevel cur_level;
	int itype;
	...
	Housekeeper_DrawALittleBlock ( hwnd,x, y, value );	
	cur_level->manx = x;
	cur_level->many = y;
