...
//static void en_kb_update(key_board_t *kb, HWND hWnd, RECT* rect)
...
static int init_en_view_window (HWND hWnd, view_window_t *vw)
{
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((vw_add_data_t *)(vw->data))->pu_normal),
				LEFT_ARROW_ENABLE)) {
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((vw_add_data_t *)(vw->data))->pu_press),
				LEFT_ARROW_DISABLE)) {
        _MY_PRINTF ("Fail to get page-up mask bitmap.\n");
        return -1;
    }
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((vw_add_data_t *)(vw->data))->pd_normal),
				RIGHT_ARROW_ENABLE)) {
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((vw_add_data_t *)(vw->data))->pd_press),
				RIGHT_ARROW_DISABLE)) {
        _MY_PRINTF ("Fail to get page-down mask bitmap.\n");
        return -1;
    }
...
static void destroy_en_view_window (view_window_t *vw)
{
    if (vw->data)
    {
        release_kbd_bitmap(&(((vw_add_data_t *)(vw->data))->pu_normal));
        release_kbd_bitmap(&(((vw_add_data_t *)(vw->data))->pu_press));
        release_kbd_bitmap(&(((vw_add_data_t *)(vw->data))->pd_normal));
        release_kbd_bitmap(&(((vw_add_data_t *)(vw->data))->pd_press));
        release_kbd_bitmap(&(((vw_add_data_t *)(vw->data))->sel_bk));
        free(vw->data);
    }
...
static int init_en_key_window (HWND hWnd, key_window_t *kw)
{
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((kw_add_data_t *)(kw->data))->char_key_press),
				CHAR_KEY_MASK)) {
    ...
    if (get_kbd_bitmap(HDC_SCREEN, 
                &(((kw_add_data_t *)(kw->data))->func_key_press),
				FUNC_KEY_MASK)) {
...
static void destroy_en_key_window (key_window_t *kw)
{
    if (kw->data)
    {
        release_kbd_bitmap (&(((kw_add_data_t *)(kw->data))->char_key_press));
        release_kbd_bitmap (&(((kw_add_data_t *)(kw->data))->func_key_press));
        free(kw->data);
    }
}
